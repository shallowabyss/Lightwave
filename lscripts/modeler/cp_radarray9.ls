...
/*
    ...
    A newer version of LW's built in radial array that will
    let you use a point in the background as the center for the
    array.

     If you are using a version of LW below version 9 you may
     have to delete the undogroupbegin();  and   undogroupend();
     tags in order to get it to work. These tags make all of the steps
     combined into one undo, instead of an undo for each operation.
*/
...
/* To cut down on the UNDO stack, this will make all of the following ONE undo */
undogroupbegin();
